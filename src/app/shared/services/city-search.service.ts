...
export class CitySearchService  {
  ...
  searchForCity(input: string) {
    this.results = [];
    if (input === '' || input.length < 3) {
      return of([]);
    } else {
      let i = 0;
      while (i < this.otherMockCities.length) {
        if (this.otherMockCities[i].city.toLowerCase().includes(input) || this.otherMockCities[i].city.includes(input)) {
          this.results.push(this.otherMockCities[i]);
        }
        i = i + 1;
      }
